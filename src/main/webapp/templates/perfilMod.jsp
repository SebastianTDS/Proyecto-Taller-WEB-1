...
<form:form action="grupos/modificar" method="POST"
	modelAttribute="formulario" class="m-3">
	...
	<div class="row align-items-center mb-2">
		...
		<div class="col-sm-10">
			<form:input path="nombre" id="nombre" type="text"
				class="form-control" value="${grupo.getNombre() }" />
		</div>
	</div>
		...
		<div class="col-sm-10">
			<c:if test="${grupo.getCerrado() }">
				<div class="form-check form-check-inline">
					<input class="form-check-input" type="radio" name="privacidad"
						id="abierto" value="<%=Privacidad.ABIERTO%>" /> <label
						class="form-check-label" for="abierto">Abierto</label>
				</div>
				<div class="form-check form-check-inline">
					<input class="form-check-input" type="radio" name="privacidad"
						id="cerrado" value="<%=Privacidad.CERRADO%>" checked /> <label
						class="form-check-label" for="cerrado">Cerrado</label>
				</div>
			</c:if>
			<c:if test="${!grupo.getCerrado() }">
				<div class="form-check form-check-inline">
					<input class="form-check-input" type="radio" name="privacidad"
						id="abierto" value="<%=Privacidad.ABIERTO%>" checked /> <label
						class="form-check-label" for="abierto">Abierto</label>
				</div>
				<div class="form-check form-check-inline">
					<input class="form-check-input" type="radio" name="privacidad"
						id="cerrado" value="<%=Privacidad.CERRADO%>" /> <label
						class="form-check-label" for="cerrado">Cerrado</label>
				</div>
			</c:if>
		</div>
	...
	<div class="row align-items-center mb-2">
		...
		<div class="col-sm-10">
			<form:input path="cantidadMax" id="cantidadMax" type="number"
				class="form-control" value="${grupo.getCantidadMax() }" />
		</div>
	</div>
	...
	<div class="row align-top mb-2">
		...
		<div class="col-sm-10">
			<textarea name="descripcion" id="descripcion" rows="10"
				class="form-control">${grupo.getDescripcion() }</textarea>
		</div>
	</div>
	...
	<div class="text-end">
		...
		<a href="grupos/${grupo.getId()}" class="btn btn-outline-danger mt-3">Descartar
			cambios</a>
	</div>
</form:form>
